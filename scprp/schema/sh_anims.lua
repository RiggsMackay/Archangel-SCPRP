...
function ix.anim.GetModelClass(model)
	...
	if (!class and (stringfind(model, "_player") or
	stringfind(model, "/acrp") or
	stringfind(model, "/player") or
	stringfind(model, "_pm") or
	stringfind(model, "/pm") or
	stringfind(model, "playermodel") or
	stringfind(model, "pm.mdl"))) then
		return "player"
	end

	if (!class and (stringfind(model, "dpfilms/metropolice"))) then
		return "metrocop"
	end

	if (!class and (stringfind(model, "/combine_"))) then
		return "overwatch"
	end

	if (!class and (stringfind(model, "/vortigaunt"))) then
		return "vortigaunt"
	end
